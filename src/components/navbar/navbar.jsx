    ...
    <>
      {inHomeSection && (
        ...
        >
          {isRoot ? (
            <Link to="/#home">
              <h1 className="font-semibold text-black text-xl">@Zaky Hafiedz</h1>
            </Link>
          ) : (
            <Link to="/#home" className="font-semibold text-black text-xl">
              @Zaky Hafiedz
            </Link>
          )}
          <ul className="flex gap-10 text-black text-xl">
            ...
            {isRoot ? (
              <li>
                <a href="#work" className="hover:text-gray-600 transition duration-300">
                  Work
                </a>
              </li>
            ) : (
            ...
            {isRoot ? (
              <Link to={"/contact"}>
                <li>
                  <a className="hover:text-gray-600 transition duration-300">Contact</a>
                </li>
              </Link>
            ) : (
              <Link to={"/contact"}>
        ...
        <>
          ...
          >
            <button onClick={toggleMenu} aria-label="Close menu">
              <X
                size={50}
                className="fixed top-[60px] right-[60px] z-40 p-2 bg-white shadow-lg rounded-full"
              />
            </button>
            <div className="flex flex-col h-full my-10 ml-20 justify-between">
              <ul className="flex flex-col gap-8 text-white text-6xl mt-32 list-none">
                {sections.map(({ id, label }) => (
                  <li key={id} className="flex flex-row gap-5 items-center">
                    {activeSection === id && <span className="text-white">•</span>}
                    <a
                      href={id}
              ...
              <ul className="absolute bottom-5 flex flex-row gap-6 text-white text-xl">
                <li>
                  <a
                    href="https://www.linkedin.com/in/zaky-hafiedz-3b1950268/"
                    className="hover:text-gray-600"
                    onClick={toggleMenu}
                  >
                    Linkedin
                ...
                <li>
                  <a
                    href="https://discord.com/users/584260346836025355"
                    className="hover:text-gray-600"
                    onClick={toggleMenu}
                  >
                    Discord
                ...
                <li>
                  <a href="#webflow" className="hover:text-gray-600" onClick={toggleMenu}>
                    Webflow
                  </a>
                </li>
              </ul>
